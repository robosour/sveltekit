...
<button on:click={() => (formModal = true)} type="submit" class="btn  btn-lg btn-ghost-accent "
	>+ District Migration</button
>
<Modal bind:open={formModal} size="xs" autoclose={false}>
	<form class="flex flex-col space-y-6" action="/migration/district/">
		<h3 class="text-xl font-medium text-gray-900 dark:text-white p-0">
			Start a new District Migration
		</h3>

		<label for="cd">District Name:</label>
		<input size="50" id="cd" type="text" name="cd" bind:value={$test.districtName} />
